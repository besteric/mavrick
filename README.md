...
- **Stuck-key prevention.** If the remote disconnects while a push-to-talk key is held, Mavrick releases the virtual key automatically.
- **Stale-hold self-heal.** If a release HID event is ever missed, the next press closes the stale hold before opening a new one.
- **HID seize.** On connect, Mavrick seizes the remote at the HID level so macOS no longer also sees media key events from it — no double-dispatch (e.g., to iTunes/Music), no system funk sound on unhandled keys.
...
2. Move `Mavrick.app` to `/Applications` (optional but helps icon caching)
3. Launch it (`open Mavrick.app`)
...
> ⚠️ **Important:** You must explicitly add **Mavrick.app** to **System Settings → Privacy & Security → Input Monitoring** (click the **+** button and select the app). Without this, Mavrick may not properly intercept HID events/media keys, which means vol up and down buttons and play/pause buttons will trigger volume change and triggering of Apple Music.

A diagnostic log is written to `/tmp/mavrick.log` (NSLog is redacted under hardened runtime, so Mavrick uses file-based logging).
